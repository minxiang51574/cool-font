<template>
	<div class="tab-chart">
		<div class="tab-chart__header">
			...
			<span class="tab-chart__year">{{ year }}</span>
		</div>
...
export default defineComponent({
	setup() {
		...
		const chartOption = reactive<any>({
			...
			tooltip: {
				...
				formatter: (comp: any) => {
					...
					const serie = comp[0],
						money = comp[2];
					return `${serie.seriesName}：${serie.value}      ${money.seriesName}：${money.value}`;
				},
				axisPointer: {
					show: true,
					status: "shadow",
		...
		chartOption.xAxis.data = new Array(12).fill(1).map((_: any, i: number) => i + 1 + "月");
		chartOption.series[0].data = new Array(12)
			.fill(1)
			.map(() => parseInt(String(Math.random() * 100)));
		chartOption.series[1].data = new Array(12).fill(100);
		chartOption.series[2].data = new Array(12).fill(100);

		const year = ref<any>("");
		onMounted(() => {
			service.totalTurnover.orderMonthChart({ dayCount: 12 }).then((res: any) => {
				chartOption.xAxis.data = res.dates;
				chartOption.series[0].data = res.datas;
				chartOption.series[2].data = res.dataPrice;
			});
			year.value = dayjs(Date.now()).format("YYYY");
		});
		return {
			chartOption,
			year
