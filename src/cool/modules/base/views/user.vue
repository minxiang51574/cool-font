					...
					<cl-crud :ref="setRefs('crud')" :on-refresh="onRefresh" @load="onLoad">
						...
						<el-row>
							...
							>
								...
								<template #column-isCs="{ scope }">
									<el-switch
										v-model="scope.row._isCs"
										active-color=" #ff4949"
										inactive-color=" #13ce66"
										@change="onIsCsChange($event, scope.row)"
									/>
								</template>
		...
		async function onRefresh(params: any, { next, render }: any) {
			...
			render(
				list.map((e: any) => {
					...
					e._isCs = e.isCs == 0 ? true : false;
					return e;
				})
			);
		}
		// 开启、关闭
		function onIsCsChange(val: boolean, item: any) {
			if (val) {
				item.isCs = 0;
			} else {
				item.isCs = 1;
			...
			service.base.system.user
				// .update({ id: item.id, isCs: item.isCs })
				.update(item)
				.then(() => {
					ElMessage.success(val ? `${item.name}取消资格` : `${item.name}成为客服`);
				})
				.catch((err: string) => {
					ElMessage.error(err);
				});
