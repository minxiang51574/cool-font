	...
	>
		...
		<ul v-loading="loading" class="cl-chat-session__list scroller1">
			<li
				...
				@click="toDetail(item)"
				@contextmenu.stop.prevent="openCM($event, item.id)"
			>
				<!-- 头像 -->
				<div class="avatar">
					<el-badge
						...
						:max="99"
					>
						<cl-avatar :size="40" :src="item.headimg" />
					</el-badge>
				</div>
...
export default defineComponent({
	setup() {
		const { store, mitt, service } = useCool();
		...
		const list = computed(() => {
			return store.getters.sessionList
				.map((e: any) => {
					return {
						...e,
						lastMessage: e.content ? parseContent(e) : ""
					};
				})
				.sort((a: any, b: any) => {
				...
				list: [
					{
						...
						callback: (_: any, done: Function) => {
							...
							store.commit("DELETE_SESSION_ITEM", id);
